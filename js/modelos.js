function Modelos()
{
	...
	this.SplashScreen = function()
	{
		...
		var geo = new THREE.BoxGeometry(8,4,0.01);
		//var mat = new THREE.MeshPhongMaterial({color:0xaaaaaa});
		
		//var image = document.createElement('img');
		//image.src = 'img/titulo.png';
		var tex = new THREE.Texture(imgTitulo);
		
/*
		var tex = new THREE.TextureLoader().load('img/titulo.png');
		tex.crossOrigin='';
*/		
		var mat = new THREE.MeshBasicMaterial( {color:0xaaaaaa, map:tex} ); 
		//var mat = new THREE.MeshBasicMaterial({map:new THREE.TextureLoader().load('img/titulo.png')});
		mat.map.needsUpdate = true;
		
		var modelo = new THREE.Mesh(geo, mat);
		geo = new THREE.BoxGeometry(8.8,4.5,0.5);
		mat = new THREE.MeshPhongMaterial({color:0xaaaaaa});
		var fondo = new THREE.Mesh(geo, mat);
		fondo.position.z = -0.5
		modelo.add(fondo);
